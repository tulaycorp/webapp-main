        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <nav class="flex mb-8 text-sm font-medium text-secondary dark:text-gray-400">
                ...
                <a href="{{ route('products') }}"
                    class="hover:text-primary dark:hover:text-white transition-colors">Products</a>
                @if($product->category)
                    <span class="mx-2">/</span>
                    <a href="{{ route('products') }}?category={{ urlencode($product->category) }}"
                        class="hover:text-primary dark:hover:text-white transition-colors">{{ $product->category }}</a>
                @endif
                <span class="mx-2">/</span>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-24">
                ...
                <div data-animate="fade-in" class="w-full">
                    ...
                    <div class="relative aspect-square bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden w-full">
                        <img id="hero-image" src="{{ $allImages->first() ?? asset('images/placeholder.png') }}"
                            alt="{{ $product->name }}" class="w-full h-full object-cover transition-opacity duration-300">
                        @if($product->featured)
                            <span
                                class="absolute top-6 left-6 px-4 py-2 bg-primary dark:bg-white text-white dark:text-gray-900 text-sm uppercase tracking-wider font-medium">Featured</span>
                        @endif
                        @if($product->on_sale)
                            <span
                                class="absolute top-6 right-6 px-4 py-2 bg-red-600 text-white text-sm uppercase tracking-wider font-medium">{{ $product->discount_percent }}%
                                Off</span>
                        @endif
                    </div>
                                ...
                                <div class="flex gap-3 py-1">
                                    @foreach($allImages as $index => $imageUrl)
                                        <button type="button"
                                            ...
                                            data-index="{{ $index }}" data-src="{{ $imageUrl }}">
                ...
                <div data-animate="slide-left" data-delay="200">
                    {{-- Vendor / Brand --}}
                    @if($product->vendor)
                        <p class="text-xs text-secondary dark:text-gray-500 uppercase tracking-widest font-medium mb-2">
                            {{ $product->vendor }}</p>
                    @endif

                    {{-- Category --}}
                    <p class="text-secondary dark:text-gray-400 uppercase tracking-widest font-medium mb-4">
                        {{ $product->category }}</p>

                    {{-- Product Name --}}
                    <h1 class="text-4xl lg:text-5xl font-impact uppercase text-primary dark:text-white mb-6 leading-tight">
                        {{ $product->name }}</h1>

                    {{-- Price Section --}}
                    <div class="flex items-center gap-6 mb-6">
                        <span
                            class="text-3xl font-bold text-primary dark:text-white">${{ number_format($product->price, 2) }}</span>
                        @if($product->compare_at_price > $product->price)
                            <span
                                class="text-xl text-secondary dark:text-gray-500 line-through">${{ number_format($product->compare_at_price, 2) }}</span>
                            <span
                                class="px-3 py-1 bg-red-100 dark:bg-red-900 text-red-600 dark:text-red-300 text-sm font-medium rounded">Save
                                ${{ number_format($product->compare_at_price - $product->price, 2) }}</span>
                        @endif
                    </div>
                        ...
                        @if($product->in_stock)
                            <span class="inline-flex items-center gap-2 text-green-600 dark:text-green-400 font-medium">
                                ...
                                @if($product->track_inventory && $product->stock_quantity <= 10 && $product->stock_quantity > 0)
                                    <span class="text-orange-500 dark:text-orange-400 text-sm">(Only {{ $product->stock_quantity }}
                                        left!)</span>
                                @endif
                            </span>
                        @else
                    ...
                    <div class="border-t border-b border-gray-200 dark:border-gray-700 mb-8">
                        ...
                        @if($product->sku || $product->barcode || $product->product_type)
                            <details class="group">
                                <summary class="py-4 flex items-center justify-between cursor-pointer list-none">
                                    <span
                                        class="font-medium text-primary dark:text-white uppercase tracking-wider text-sm">Product
                                        Details</span>
                                    <i data-lucide="chevron-down"
                                        class="w-5 h-5 text-secondary transition-transform group-open:rotate-180"></i>
                                </summary>
                        ...
                        @if($product->requires_shipping && ($product->weight || $product->length || $product->width || $product->height))
                            <details class="group border-t border-gray-200 dark:border-gray-700">
                                <summary class="py-4 flex items-center justify-between cursor-pointer list-none">
                                    <span
                                        class="font-medium text-primary dark:text-white uppercase tracking-wider text-sm">Shipping
                                        Information</span>
                                    <i data-lucide="chevron-down"
                                        class="w-5 h-5 text-secondary transition-transform group-open:rotate-180"></i>
                                </summary>
                                <div class="pb-4 text-sm text-secondary dark:text-gray-400 space-y-2">
                                    ...
                                    @if($product->length || $product->width || $product->height)
                                        <div class="flex justify-between">
                                            ...
                                            <span>{{ $product->length ?? '-' }} x {{ $product->width ?? '-' }} x
                                                {{ $product->height ?? '-' }} {{ $product->dimension_unit }}</span>
                                        </div>
                                    @endif
                                </div>
                            </details>
                        ...
                        @if($product->tags)
                            <details class="group border-t border-gray-200 dark:border-gray-700">
                                <summary class="py-4 flex items-center justify-between cursor-pointer list-none">
                                    <span
                                        class="font-medium text-primary dark:text-white uppercase tracking-wider text-sm">Tags</span>
                                    <i data-lucide="chevron-down"
                                        class="w-5 h-5 text-secondary transition-transform group-open:rotate-180"></i>
                                </summary>
                                <div class="pb-4">
                        ...
                        @if($product->metafields && count($product->metafields) > 0)
                            <details class="group border-t border-gray-200 dark:border-gray-700">
                                <summary class="py-4 flex items-center justify-between cursor-pointer list-none">
                                    <span
                                        class="font-medium text-primary dark:text-white uppercase tracking-wider text-sm">Additional
                                        Information</span>
                                    <i data-lucide="chevron-down"
                                        class="w-5 h-5 text-secondary transition-transform group-open:rotate-180"></i>
                                </summary>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        @if(!$product->track_inventory || $product->stock_quantity > 0 || $product->continue_selling_when_out_of_stock)
                            <button data-add="{{ $product->id }}" data-stock="{{ $product->stock_quantity }}"
                                ...
                                class="flex-1 px-8 py-4 bg-primary dark:bg-white text-white dark:text-gray-900 text-base uppercase tracking-wider font-medium hover:opacity-90 transition-opacity text-center disabled:opacity-50 disabled:cursor-not-allowed">
                                Add to Cart
                            </button>
                        @else
                            <button disabled
                        ...
                        {{-- Share Button --}}
                        <button id="share-btn" onclick="(function(btn) {
            ...
            @if($frequentlyBought->count() > 0)
                <div class="mb-24">
                    <h2 class="text-2xl font-impact uppercase text-primary dark:text-white mb-10 tracking-wide">Frequently
                        Bought Together</h2>
            ...
            @if($suggested->count() > 0)
                <div>
                    <h2 class="text-2xl font-impact uppercase text-primary dark:text-white mb-10 tracking-wide">You Might Also
                        Like</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
